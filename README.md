> [!NOTE]
> This repository is a local GNU coreutils checkout with additional migration
> work for building `coreutils` as `wasm32-wasip1` binaries with `wasi-sdk` on
> macOS.
>
> For the WASI-specific work in this tree, start with these documents:
> - `Migration.md`: migration status, environment, tool versions, issues, and fixes
> - `README-wasi-runtime.md`: runtime status and tested commands under Wasmer 7
> - `scripts/wasi-build.sh`: WASI configure and build entry point
> - `scripts/wasi-runtime-tests.sh`: collected runtime test commands
>
> Acknowledgements:
> - GNU coreutils maintainers and contributors for the upstream project
> - OpenAI Codex for the local WASI migration, build fixes, runtime testing, and documentation work in this repository
